...
import { updateAddress, loadAddress } from '../../features/actions/addressAction';
...
const UpdateAddress = () => {
    ...
    const filteredAddress = address.filter((item) => item.uid === uid);
    
    // Checking if the filtered array has any items
    const data = filteredAddress.length > 0 ? filteredAddress[0] : null;
    ...
    const onSubmit = async (event) => {
        ...
        setValues({ ...values });
        // console.log("check 1")
        const resultAction = await dispatch(updateAddress({ type, address1, landmark, city, state, zipcode, country, uid }));
    ...
    return (
        <Layout>
            <div className='w-1/2 mx-auto my-5 bg-blur bg-gray-100 rounded-lg'>
                <h1 className='text-gray-500 text-xl text-center font-bold mt-2'>Update Your Address</h1>
