...
import { laodProfile, updateProfile } from '../features/actions/authActions';
...
const UpdateAccount = () => {
  ...
  const { first_name, last_name, username, phone, gender } = values;
  ...
  const onSubmit = async(event) => {
      ...
      const resultAction = await dispatch(updateProfile({ first_name, last_name, username, profilePic, phone, gender}));
  ...
  return (
      <div className="flex justify-center mx-auto py-20 min-h-screen rounded-lg md:flex">
          <div className="flex w-1/2 justify-center py-10 items-center bg-gray-100 shadow-lg rounded-lg">
              <form className="bg-white p-3 rounded-lg" >
                  <h1 className="text-gray-800 font-bold text-2xl mb-1">Update Details</h1>
